...
- **每小时提醒**：顶栏铃铛开关，整点对齐提醒；当前小时已记录则不打扰。
  **通知内直接点表情快速记录**，不用打开 app。用精确闹钟实现，关掉 app 或重启手机后仍会提醒
  用精确闹钟实现，关掉 app 或重启手机后仍会提醒（详见下方「提醒可靠性」）
- **设置页**：主题模式、主题色（5 色 / 跟随壁纸取色）、启动时打开的页面、
  点日历的行为、日历格子显示备注、免打扰时段、默认心情、每周起始日、清空数据
- **测试通知**：设置页可立即弹出一条带表情的通知，验证提醒样式与交互
...
定时提醒用 **AlarmManager 精确闹钟**（`setExactAndAllowWhileIdle`），
而不是 WorkManager 周期任务 —— 后者在系统 Doze 下会被推迟，
国产 ROM 的电池优化更是直接拦截，实测表现为「只有 app 在后台才提醒」。

- 开机 / 应用升级后由广播自动恢复调度
- Android 12+ 会先检查精确闹钟权限，无权限时自动退回普通重复闹钟（不崩溃）
- 设置页「提醒可靠性」会检测是否已加入**电池优化白名单**，未加入可一键跳转设置
  —— 这一步在小米 / 华为 / OPPO 等 ROM 上是提醒能否送达的关键
